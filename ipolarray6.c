	      ...
	      double complex N_coord[NDIM][NDIM])
{
    ...
    //source step only if matter is along the way
    if(Ne > 0){
      gcov_func(Xf,gcov);                                                                                                
    ...
    /*** 1. combined emission/absorption step ***/
    double ap2 = aQ*aQ + aU*aU + aV*aV;
    double ap = sqrt(ap2);
    double nQ = sqrt(aQ*aQ + aV*aV);
    double nU = sqrt(aU*aU + aV*aV);
    double norm1,norm2;

    //eigen vectors and corresponding eigen values
    norm1=sqrt(2.*(aQ*aU*nQ*nU + nQ*nQ*nU*nU));
    ...
    if(norm1>0.0){
      ...
      e1[1] = -aV*nU/norm1;
      e1[2] = -aV*nQ/norm1;
      e1[3] = (aU*nQ + aQ*nU)/norm1;
    }
    
    norm2=sqrt(2.*(-aQ*aU*nQ*nU + nQ*nQ*nU*nU));
    ...
    if(norm2>0.0){
      ...
      e2[1] = aV*nU/norm2;
      e2[2] = -aV*nQ/norm2;
      e2[3] = (aU*nQ - aQ*nU)/norm2;
    }
    

    double e3[] = {1./S2, 0.0, 0.0, 0.0};
    double l3 = aI ;
    double e4[] = {-1./S2, 0.0, 0.0, 0.0};
    double l4 = aI ;
    ...
    double j1,j2,j3,j4;
    double a10,a20,a30,a40;
    double a1,a2,a3,a4;

    //project S and jS onto eigen basis vectors (dot products of S and jS  with each basis vector).
    //and evolve the amplitudes of eigenvector
    j1=jI*e1[0] + jQ*e1[1] + jU*e1[2] + jV*e1[3];
    a10=SI0*e1[0] + SQ0*e1[1] + SU0*e1[2] + SV0*e1[3];
    a1=j1/l1*(1.-exp(-l1*dlam))+a10*exp(-l1*dlam);

    j2=jI*e2[0] + jQ*e2[1] + jU*e2[2] + jV*e2[3];
    a20=SI0*e2[0] + SQ0*e2[1] + SU0*e2[2] + SV0*e2[3];
    a2=j2/l2*(1.-exp(-l2*dlam))+a20*exp(-l2*dlam);
    
    j3=jI*e3[0] + jQ*e3[1] + jU*e3[2] + jV*e3[3];
    a30=SI0*e3[0] + SQ0*e3[1] + SU0*e3[2] + SV0*e3[3];
    a3=j3/l3*(1.-exp(-l3*dlam))+a30*exp(-l3*dlam);
    
    j4=jI*e4[0] + jQ*e4[1] + jU*e4[2] + jV*e4[3];
    a40=SI0*e4[0] + SQ0*e4[1] + SU0*e4[2] + SV0*e4[3];
    a4=j4/l4*(1.-exp(-l4*dlam))+a40*exp(-l4*dlam);

    /*
    fprintf(stdout,"N1: %g  N2: %g  \n",norm1,norm2);
    fprintf(stdout,"e1: %g %g %g %g \n",e1[0],e1[1],e1[2],e1[3]);
    fprintf(stdout,"e2: %g %g %g %g \n",e2[0],e2[1],e2[2],e2[3]);
    fprintf(stdout,"e3: %g %g %g %g \n",e3[0],e3[1],e3[2],e3[3]);
    fprintf(stdout,"e4: %g %g %g %g \n",e4[0],e4[1],e4[2],e4[3]);
    fprintf(stdout,"alphaIQUV:  %g %g %g %g \n",aI,aQ,aU,aV);
    fprintf(stdout,"ji:  %g %g %g %g \n",j1,j2,j3,j4);
    fprintf(stdout,"ai0: %g %g %g %g \n",a10,a20,a30,a40);
    fprintf(stdout,"ai:  %g %g %g %g \n",a1,a2,a3,a4);
    */

    // reconstruct the Stokes parameters using the amplitudes and basis vectors
    SI1 = a1*e1[0] + a2*e2[0] + a3*e3[0] + a4*e4[0];
    SQ1 = a1*e1[1] + a2*e2[1] + a3*e3[1] + a4*e4[1];
    SU1 = a1*e1[2] + a2*e2[2] + a3*e3[2] + a4*e4[2];
    SV1 = a1*e1[3] + a2*e2[3] + a3*e3[3] + a4*e4[3];
    

    /*** 2. analytical rotation/conversion step ***/
    
    rdS=rQ*SQ1+rU*SU1+rV*SV1;
    ...
    if(rho>0.0){
      SI=SI1;
      SQ=SQ1*c + 2*rQ*rdS/rho2*sh*sh + (rU*SV1-rV*SU1)/rho*s;
      SU=SU1*c + 2*rU*rdS/rho2*sh*sh + (rV*SQ1-rQ*SV1)/rho*s;
      SV=SV1*c + 2*rV*rdS/rho2*sh*sh + (rQ*SU1-rU*SQ1)/rho*s;
    }else{
      SI=SI1;
      SQ=SQ1;
      SU=SU1;
