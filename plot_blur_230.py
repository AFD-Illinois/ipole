...
ax.pcolormesh(X*M_to_uas, Y*M_to_uas, Z/Z.max(), cmap='afmhot', vmin=0, vmax=1)
ax.set_aspect('equal')
ax.set_xlim([-50,50])
ax.set_ylim([-50,50])
ax.set_xlabel('X [uas]')
...
sigma = 15/2/uas_per_pix/np.sqrt(8*np.log(2.))
#Z = rotate(Z, 16, reshape=False)
Z = gf(Z, sigma, truncate=3.0)
#Z = rotate(Z, -16, reshape=False)
#Z = np.log10(Z/Z.max())
cax = ax.pcolormesh(X*M_to_uas, Y*M_to_uas, Z/Z.max(), cmap='afmhot', vmin=0, vmax=1)
fig.colorbar(cax, ax=axes.ravel().tolist(), label='Intensity [arb]')
