...
static inline int levi_civita(int i, int j, int k, int l)
{
...
*/
static inline int LU_decompose(double A[][NDIM], int permute[])
{
...
*/
static inline void LU_substitution(double A[][NDIM], double B[], int permute[])
{
  int i, j;
  int n = NDIM;
  double tmpvar;
...
/*
 ...
 Returns (1) if a singular matrix is found,  (0) otherwise.
 */
static inline int invert_matrix(double Am[][NDIM], double Aminv[][NDIM])
{
