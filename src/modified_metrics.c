...
/*Return the Einstein dilaton Gauss Bonet metric in KS coordinates
   The value of zeta determines the deviation from GR
     zeta should be bound from 0 to 0.3*/
  double cth = cos(th);
  double sth = sin(th);
  double s2 = sth * sth;
...
//0 used to replace a*a2
  ...
  gcov[2][2] = pow(r,2) + a2*c2 + ((8820000. - 6213200.*r - 3416700.*pow(r,2) - 1855650.*pow(r,3) + 887110.*pow(r,4) + 800733.*pow(r,5) + 435540.*pow(r,6) 
  ...
  gcov[3][3] =-(pow(r,2)*(-1. + c2)) - (a2*(2. + r - 2.*c2)*(-1. + c2))/r - (2.*a4*c2*pow((-1. + c2),2))/pow(r,3) 
