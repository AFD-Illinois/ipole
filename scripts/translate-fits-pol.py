...
user = "C.F. Gammie"
data_to_save = "polarized"  # "polarized" or "unpolarized"
...
def write_fits_from_ipole(fname, ofname):
  ...
  data = np.rot90(I_Jy,krotate) * scale
  Qdata = np.rot90(hfp['pol'][:,:,1].T) * scale
  Udata = np.rot90(hfp['pol'][:,:,2].T) * scale
  Vdata = np.rot90(hfp['pol'][:,:,3].T) * scale
