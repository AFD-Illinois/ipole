...
void init_model(Params *params)
{
  ...
  METRIC_MKS3 = 0;
  hslope = 1.0;
  Rh = 0.1; // Don't integrate past the coordinate origin, spherical coordinates don't like that
  ...
  stokes_I = calloc(params->maxnstep, sizeof(double));
  stokes_Q = calloc(params->maxnstep, sizeof(double));
  stokes_U = calloc(params->maxnstep, sizeof(double));
  stokes_V = calloc(params->maxnstep, sizeof(double));
  lambda = calloc(params->maxnstep, sizeof(double));;
...
void output_hdf5()
{
  ...
  hdf5_set_directory("/");
  hsize_t stokes_dims[1] = {MAXNSTEP};
